...
void light_adjust_RGBWC_hw(unsigned char red,unsigned char green,unsigned char blue,unsigned char warm,unsigned char white,unsigned char lum)
{
    ...
    tmp_r = red * PWM_FRQ_SELECTION / 255;
    tmp_g = green * PWM_FRQ_SELECTION / 255;
    tmp_b = blue * PWM_FRQ_SELECTION / 255;
	
	pwm_set_duty(PWM_CHANNEL_RGB_R, tmp_r);
	pwm_set_duty(PWM_CHANNEL_RGB_G, tmp_g);
	pwm_set_duty(PWM_CHANNEL_RGB_B, tmp_b);
}
...
void light_adjust_HSV_hw(uint16_t hue,uint16_t saturation,uint16_t value){
	static uint16_t pre_h=0,pre_s=0,pre_v=0;
...
void scene_run(void){
	...
	static u32 T = 0;
	uint8_t scene = light_value.scene_mode;
