...
typedef struct 
{
    ...
    uint8_t ucUnitCnt;
	uint8_t uiGainTemp;
    LIGHT_CONTRL_T tTargetVal;
	LIGHT_CONTRL_T tCurrVal;
}LIGHT_CTRL_HANDLE_T;
...
// 连续重置5次重新进入正在配网状态，每次相隔超过4s时重新计算配网次数
...
{
	static uint16_t r=0,g=0,b=0,c=0,w=0;
...
void ty_light_gradual_ctrl_cw(u16 usBright, u16 usTemperatue)
{
	...
	if(usBright < 10){
		...
		sence_ctrl_data.tTargetVal.usWarm = usTemperatue;			
	}else{
		...
		sence_ctrl_data.tTargetVal.usCold = usBright;
		sence_ctrl_data.tTargetVal.usWarm = usTemperatue;	
...
//		sence_ctrl_data.tTargetVal.usWarm = usBright - sence_ctrl_data.tTargetVal.usCold ;		
	...
	light_adjust_RGBWC_hw(sence_ctrl_data.tTargetVal.usRed,sence_ctrl_data.tTargetVal.usGreen,sence_ctrl_data.tTargetVal.usBlue,sence_ctrl_data.tTargetVal.usWarm,sence_ctrl_data.tTargetVal.usCold);	
...
light_adjust_RGBWC_hw(0,0,0,0,0);	
		...
		case SCENE_MODEL:{
			...
			if(hal_clock_time_exceed(sence_uint_cnt_time,compare_time)){
				...
				switch(light_value.sence_model){
					case SCENE_MODEL0:{
						//uint16_t light_value_T=0,light_value_B=200;
						ty_light_gradual_ctrl_cw(200, 0);
					}break;
			
					case SCENE_MODEL3:{
						//uint16_t light_value_T=500,light_value_B=500;
						ty_light_gradual_ctrl_cw(500, 500);
					}break;
...
// 遥控器事件处理
void remote_control_pair_handle(unsigned char dplen, unsigned char *dpvalue)
{
    const uint8_t light_model = dpvalue[0];
    const int16_t bright_value = dpvalue[1] << 8 | dpvalue[2];
    uart_putc(light_model);
    uart_putc(bright_value);

    switch (light_model) {
        case 0x00: // 白光模式
            light_value.scene_mode = SCENE_WHITE;
            light_value.B = bright_value;
            break;
        case 0x01: // 彩色模式
            light_value.scene_mode = SCENE_COLOR;
            light_value.V = bright_value;
            break;
        case 0x02: // 场景模式
            light_value.scene_mode = SCENE_MODEL;
            light_value.V = bright_value;
...
void app_light_run(void){
    ...
    if(light_time_setok_flag > 0){
	 ...
	 // 如果是正在配网状态，灯闪烁，每500ms闪烁1次，配网状态切换在组件层切换
	if(ty_beacon_get_state() == DEVICE_STATE_PAIRING){
		...
		if(hal_clock_time_exceed_rtc(T,500000)){			
			...
			blink_B = blink_B == 0?1000:0; 
			ty_light_gradual_ctrl_cw(blink_B, 1000);
